...
BATCH_SIZE  = 600#51201     # VAEs are more sensitive to batch size than standard autoencoders because:
...
class GPUTensorLoader:
    """
    Minimal DataLoader replacement that keeps the whole dataset resident on
    DEVICE and does shuffling/batching with GPU tensor ops, so no batch ever
    has to cross the CPU-GPU boundary during training.
    """
    def __init__(self, data, targets, device, batch_size, shuffle=True):
    ...
    def __len__(self):
        return (self.data.size(0) + self.batch_size - 1) // self.batch_size
...
def run(beta: float = BETA, epochs: int = EPOCHS, fm_epochs: int = EPOCHS_FM,
    ...
    # happens entirely on GPU via GPUTensorLoader.
    train_set = datasets.MNIST("./data", train=True,  download=True)
    test_set  = datasets.MNIST("./data", train=False, download=True)

    train_x = train_set.data.float().div(255.0).unsqueeze(1)   # (N,1,28,28)
    train_y = train_set.targets
    test_x  = test_set.data.float().div(255.0).unsqueeze(1)
    test_y  = test_set.targets

    train_loader = GPUTensorLoader(train_x, train_y, DEVICE, batch_size=BATCH_SIZE, shuffle=True)
    test_loader  = GPUTensorLoader(test_x,  test_y,  DEVICE, batch_size=BATCH_SIZE, shuffle=True)
    print("DEVICE=", DEVICE)
